...
export default function LongAnswersPage() {
  ...
  const handleNext = () => {
    ...
    const longAnswersSchema = z.object(schemaShape);
    const result = longAnswersSchema.safeParse(longAnswers);

    if (!result.success) {
      const unansweredCount = questions.filter(q => !longAnswers[q.id]?.trim()).length;
            ...
            <div key={q.id} className="w-full flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-3 w-full">
                <span className="text-[16px] font-semibold text-[#0D1A14] leading-[24px]" style={{ fontFamily: "var(--font-inter-tight)" }}>
                  Question {questionIndex}: <span className="text-[#FF4242] ml-0.5 font-semibold">*</span>
                </span>
                {q.description && (
